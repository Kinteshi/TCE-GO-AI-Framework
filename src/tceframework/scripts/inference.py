...
from tceframework.data.preprocessing.classification import pp_second_tabular_inference, pp_tabular_inference, pp_svm_inference
...
def svm_inference(filters: dict):
    config.CHANGE_ROOT_DIR('PRODUCTION')
    ...
    X = pp_svm_inference(data.copy())
    model = load_model(filename='svm_model.pkl')
    y_pred_class = model.predict(X)
    del model, X
    gc.collect()
